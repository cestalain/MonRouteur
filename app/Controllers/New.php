...
$twig = new Environment($loader, ['cache' => false, 'debug' => false, ]);
$twig->addExtension(new DebugExtension);

/* CONFIGURATION (valeurs a personnaliser) */
// Chemin d'acces du dossier des images originales et des miniatures
$theme_encours = LESTHEMES[$_SESSION['theme']];
//$oripath = '../public/Images/aquarelles/' . $theme_encours . '/';
$originalesPath = '..'.rtrim(dirname($_SERVER['PHP_SELF']), '/\\').'/Images/aquarelles/'. $theme_encours . '/'; // Images originales
//$originalesPath = $oripath;
$originalesUrl = rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/"; // Images originales
$originalesUrl = $originalesPath;//$_SERVER['DOCUMENT_ROOT'] . $originalesPath;
//$originalesPath=$originalesUrl;
//$originalesPath=rtrim(dirname($_SERVER['PHP_SELF']), '/\\'). $originalesPath . LESTHEMES[$_SESSION['theme']];
//$flag = opendir($miniaturesUrl . 'marine/');
//$originalesUrl = rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/Images/aquarelles"; // Images originales
//$miniaturesPath = $miniaturesP; // Miniatures
//$miniaturesUrl = $miniaturesP; // Miniatures
...
$miniaturesPath = $originalesPath . "mini/";
$miniaturesUrl = $miniaturesPath ;//$originalesUrl . "mini/";
$repertoiresPath = array($miniaturesPath, $originalesPath);
...
if (is_dir($miniaturesUrl)) {
    $source = 1;
} else {
    list($source, $lienMiniOk) = array(0, 0);
}
// Si varialge "mode" presente dans l'Url, alors on adapte le fonctionnement
if (isset($_GET['mode'])) {
    list($lienOrig, $lienMini) = array('orig_d', 'mini_c');
    if ($_GET['mode'] == 'orig_c') {
        list($source, $triOk, $lienOrig) = array(0, 0, 'orig_d');
    }
    if ($_GET['mode'] == 'orig_d') {
        list($source, $triOk, $lienOrig) = array(0, 1, 'orig_c');
    }
    if ($_GET['mode'] == 'mini_c') {
        list($source, $triOk, $lienMini) = array(1, 0, 'mini_d');
    }
    if ($_GET['mode'] == 'mini_d') {
        list($source, $triOk, $lienMini) = array(1, 1, 'mini_c');
    }
} else {
    list($lienOrig, $lienMini) = array('orig_d', 'mini_c');
}
...
if ($_SESSION['theme']) {
    ...
    switch ($_SESSION['theme']) {
        case "M":
            // case "M":
            $dossier_encours = $originalesUrl;
            $mini = $originalesUrl . 'mini/';
            $repertoiresUrl = array($originalesUrl,$mini);
            $dossier = getcwd();
            $img = glob($mini . "*.jpg");
        break;
        case "P":
    ...
    if ($doss = opendir($mini)) {
        ...
        */
        if ($triOk) {
            rsort($liste);
        } // On tri la liste d'images par ordre decroissant
        ...
        if ($source) {
            for ($i = 0;$i < count($liste);$i++) {
                ...
                $imageOrigPath = $repertoiresPath[$source] . $liste[$i]; // Chemin de l'image originale
                $imageOrigUrl = $repertoiresUrl[$source] . $liste[$i]; // Chemin de l'image originale
                // Si l'image originale existe, alors on contruit le lien, sinon on affiche juste la miniature
                if (file_exists($repertoiresPath[$source].$liste[$i])) {
               // if (file_exists($dossier_encours . $liste[$i])) {
                    $image = '<td class = "lolo"><a href="' . $imageOrigPath . '" title="' . $liste[$i] . '" target="_blank"';
                    if ($jsOk) {
                        list($largeur, $heuteur) = getimagesize($dossier_encours . $liste[$i]);
...
?>
        ...
        <head>
            ...
            <script type='text/javascript'>
                ...
                function imgPop(theImage, theWidth, theHeight, theTitle) {
                    ...
                    var theWidthWin = theWidth + 20;
                    var theHeightWin = theHeight + 20;
                    // Dimensions de l'ecran
                    var theWidthScreen = screen.width;
                    var theHeightScreen = screen.height;
                    // Position de la nouvelle fenetre popup
                    var leftPosition = (theWidthScreen - theWidthWin) / 2;
                    var topPosition = (theHeightScreen - theHeightWin) / 2;
                    // Correction des valeurs selon les dimensions de l'ecran
                    if (theWidthWin > theWidthScreen) {
                        theWidthWin = theWidthScreen;
                        theHeightWin = theHeightWin + 15;
                        leftPosition = 0;
                    }
                    if (theHeightWin > theHeightScreen) {
                        theHeightWin = theHeightScreen;
                        theWidthWin = theWidthWin + 15;
                        topPosition = 0;
                    }
        ...
        if ($lienMiniOk || $lienOrigOk) {
            echo '<p>';
            if ($lienMiniOk) {
                echo '-&nbsp;<a href="?mode=' . $lienMini . '" title="Affichage des miniatures">Miniatures</a>&nbsp;-';
            }
            if ($lienOrigOk) {
                echo '-&nbsp;<a href="?mode=' . $lienOrig . '" title="Affichage des images originales">Originales</a>&nbsp;-';
            }
            echo '</p>' . "\n";
        }
        // On affiche le tableau avec les images
        //var_dump($table);
        //echo $twig->render('test.twig', ['table' => $table]);
        $matable = implode("\n", $table) . "\n";
        echo $twig->render('aquarelle.twig', ['table' => $matable]);
        ?>
        </div>
        </body>
