...
$originalesPath = rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . '/Images/aquarelles/'; // Images originales
$originalesUrl = rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/"; // Images originales

$originalesUrl = $_SERVER['DOCUMENT_ROOT'] . $originalesPath;
$originalesPath = rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/Images/aquarelles/";

$miniaturesPath = $originalesPath . 'mini/'; // Miniatures

$x = dirname($_SERVER['PHP_SELF']);

$op = rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . "/Images/aquarelles/";
$miniaturesUrl = $_SERVER['DOCUMENT_ROOT'] . $op . 'mini/'; // Miniatures



$miniaturesP = $_SERVER['DOCUMENT_ROOT'] . $op . 'mini/'; // Miniatures


$flag = opendir($miniaturesUrl . 'marine/');
if (is_dir($miniaturesUrl . 'marine/')) {
	$source = 1;
}
...
if ($_SESSION['theme']) {
	...
	switch ($_SESSION['theme']) {
		case "M":
			$dossier = 'marine/';
			$miniaturesPath = $miniaturesPath . 'marine/';
			$miniaturesUrl = $miniaturesUrl . 'marine/';
			//$repertoiresPath = array($originalesPath, $miniaturesPath);
			$repertoiresUrl = array($originalesUrl, $miniaturesUrl);
			$dossier = getcwd();
			$img = glob($dossier . "./Images/aquarelles/marine/mini/*.jpg");
			break;
		case "P":
	...
	$flag = opendir($originalesUrl);

	if ($dossier =  opendir($originalesUrl)) {
	...
	*/
		...
		if ($source) {
			for ($i = 0; $i < count($liste); $i++) {
				...
				;
				if (file_exists("../public/Images/aquarelles/" . $liste[$i])) {
